...
#-3. Write a program that prints a message if a variable is less than or equal to 10, and another if the variable is greater than 10 but less than or equal to 25, and another if the variable is greater than 25
...
def retRemainder(x, y):
    return x % y
#-5. Create a program that takes two variables divides them and returns the quotient
def retQuotient(x, y):
    return x/y
#-6. Write a program with a variable "age" assigned to an integer that prints different strings depending on the value of the integer
def ageProps(age):
    if age < 2:
        print("Baby")
